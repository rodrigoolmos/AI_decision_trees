...
#define SLD_TREES 0x88
#define DEV_NAME             "sld,trees_vivado"

/* <<--params-->> */
const int32_t read_inference = 1;
const int32_t load_features = 32;
const int32_t load_trees = 32768;
...
static void init_buf(token_t *in, token_t *gold)
{
    ...
    for (i = 0; i < 1; i++)
        for (j = 0; j < load_trees+load_features; j++)
...
int main(int argc, char *argv[])
{
    ...
    ndev = probe(&espdevs, VENDOR_SLD, SLD_TREES, DEV_NAME);
    if (ndev == 0) {
    ...
    for (n = 0; n < ndev; n++) {
        ...
        for (coherence = ACC_COH_NONE; coherence <= ACC_COH_RECALL; coherence++) {
            ...
            /* <<--regs-config-->> */
			iowrite32(dev, TREES_READ_INFERENCE_REG, read_inference);
			iowrite32(dev, TREES_LOAD_FEATURES_REG, load_features);
			iowrite32(dev, TREES_LOAD_TREES_REG, load_trees);
