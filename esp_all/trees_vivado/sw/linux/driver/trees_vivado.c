...
#define DRV_NAME "trees_vivado"

/* <<--regs-->> */
#define TREES_READ_INFERENCE_REG 0x48
#define TREES_LOAD_FEATURES_REG 0x44
#define TREES_LOAD_TREES_REG 0x40

struct trees_vivado_device {
...
static struct of_device_id trees_device_ids[] = {
    {
        .name = "SLD_TREES_VIVADO",
    },
    {
        .name = "eb_88",
    },
    {
        .compatible = "sld,trees_vivado",
    },
    {},
};
...
static inline struct trees_vivado_device *to_trees(struct esp_device *esp)
{
    return container_of(esp, struct trees_vivado_device, esp);
}

static void trees_prep_xfer(struct esp_device *esp, void *arg)
{
    struct trees_vivado_access *a = arg;

    /* <<--regs-config-->> */
	iowrite32be(a->read_inference, esp->iomem + TREES_READ_INFERENCE_REG);
	iowrite32be(a->load_features, esp->iomem + TREES_LOAD_FEATURES_REG);
...
{
    /* struct trees_vivado_device *trees = to_trees(esp); */
    /* struct trees_vivado_access *a = arg; */
...
{
    struct trees_vivado_device *trees;
    struct esp_device *esp;
    int rc;
...
static struct esp_driver trees_driver = {
    ...
    .ioctl_cm      = TREES_VIVADO_IOC_ACCESS,
    .arg_size      = sizeof(struct trees_vivado_access),
};
...
MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("trees_vivado driver");
